...
public abstract class LinuxPresetManager {
    public static EnvVars getEnvVars(Context context, String id) {
        EnvVars envVars = new EnvVars();

        if (id.equals(LinuxPreset.STABILITY)) {
        ...
        else if (id.equals(LinuxPreset.CONSERVATIVE)) {
            envVars.put("BOX64_DYNAREC_SAFEFLAGS", "2");
            envVars.put("BOX64_DYNAREC_FASTNAN", "0");
            envVars.put("BOX64_DYNAREC_FASTROUND", "0");
            envVars.put("BOX64_DYNAREC_X87DOUBLE", "1");
        ...
        else if (id.equals(LinuxPreset.INTERMEDIATE)) {
            envVars.put("BOX64_DYNAREC_SAFEFLAGS", "2");
        ...
        else if (id.equals(LinuxPreset.PERFORMANCE)) {
        ...
        else if (id.startsWith(LinuxPreset.CUSTOM)) {
    ...
    public static ArrayList<LinuxPreset> getPresets(Context context) {
        ArrayList<LinuxPreset> presets = new ArrayList<>();
        presets.add(new LinuxPreset(LinuxPreset.STABILITY, context.getString(R.string.stability)));
        presets.add(new LinuxPreset(LinuxPreset.CONSERVATIVE, context.getString(R.string.conservative)));
        presets.add(new LinuxPreset(LinuxPreset.INTERMEDIATE, context.getString(R.string.intermediate)));
        presets.add(new LinuxPreset(LinuxPreset.PERFORMANCE, context.getString(R.string.performance)));
        for (String[] preset : customPresetsIterator(context)) presets.add(new LinuxPreset(preset[0], preset[1]));
        return presets;
    }

    public static LinuxPreset getPreset(Context context, String id) {
        for (LinuxPreset preset : getPresets(context)) if (preset.id.equals(id)) return preset;
        return null;
    }
    ...
    public static int getNextPresetId(Context context) {
        int maxId = 0;
        for (String[] preset : customPresetsIterator(context)) {
            maxId = Math.max(maxId, Integer.parseInt(preset[0].replace(LinuxPreset.CUSTOM+"-", "")));
        }
        return maxId+1;
    }
    ...
    public static void editPreset(Context context, String id, String name, EnvVars envVars) {
        ...
        if (id != null) {
            ...
            String preset = LinuxPreset.CUSTOM+"-"+getNextPresetId(context)+"|"+name+"|"+envVars.toString();
    ...
    public static void duplicatePreset(Context context, String id) {
        ArrayList<LinuxPreset> presets = getPresets(context);
        LinuxPreset originPreset = null;
        for (LinuxPreset preset : presets) {
    ...
    public static String getSpinnerSelectedId(Spinner spinner) {
        ...
        if (adapter != null && adapter.getCount() > 0 && selectedPosition >= 0) {
            return ((LinuxPreset)adapter.getItem(selectedPosition)).id;
        }
        else return LinuxPreset.DEFAULT;
    }
}
